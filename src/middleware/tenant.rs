...
impl FromRequestParts<AppState> for TenantContext {
    ...
    async fn from_request_parts(parts: &mut Parts, state: &AppState) -> Result<Self, Self::Rejection> {
        ...
        if let Some(h) = parts.headers.get("x-workspace-id").and_then(|v| v.to_str().ok()) {
            if let Ok(parsed) = Uuid::parse_str(h) { target_workspace_id = Some(parsed); }
        }
            ...
            if let Some(query) = parts.uri.query() {
                for pair in query.split('&') {
                    ...
                    if let (Some(k), Some(v)) = (kv.next(), kv.next()) {
                        if k == "workspaceId" {
                            if let Ok(parsed) = Uuid::parse_str(v) { target_workspace_id = Some(parsed); break; }
                        }
                    }
                }
            }
        }
        if target_workspace_id.is_none() { target_workspace_id = claims.workspace_id; }
        ...
        Ok(TenantContext { workspace_id, user_id: claims.sub, user_role: claims.role, workspace_role })
    }
}
