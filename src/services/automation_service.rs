...
fn render_string(s:&str,input:&Value)->String{let mut out=s.to_string();let mut pos=0;while let Some(i)=out[pos..].find("{{$json."){let start=pos+i;if let Some(e)=out[start..].find("}}"){let end=start+e;let token=&out[start..end+2];let path=&out[start+8..end];let replacement=match lookup(input,path){Value::String(v)=>v.clone(),v=>v.to_string()};out=out.replacen(token,&replacement,1);pos=start+1}else{break}}out}
...
pub fn validate_graph(g:&AutomationGraph)->Result<(),AppError>{
 ...
 for n in &g.nodes{if !matches!(n.node_type.as_str(),"trigger.manual"|"trigger.webhook"|"trigger.schedule"|"data.set"|"logic.condition"|"logic.if"|"logic.switch"|"logic.filter"|"logic.merge"|"transform.merge"|"transform.split"|"transform.aggregate"|"http.request"|"action.http"|"ai.generate"|"ai.agent"|"delay.wait"|"data.noop"){return Err(AppError::BadRequest(format!("Unsupported automation node type: {}",n.node_type)))}}for e in &g.edges{if !ids.contains(e.source.as_str())||!ids.contains(e.target.as_str()){return Err(AppError::BadRequest("Automation edge references unknown node".into()))}if e.source==e.target{return Err(AppError::BadRequest("Automation cannot self-loop".into()))}} let mut indegree:HashMap<&str,usize>=g.nodes.iter().map(|n|(n.id.as_str(),0)).collect(); let mut adj:HashMap<&str,Vec<&str>>=HashMap::new(); for e in &g.edges{*indegree.get_mut(e.target.as_str()).unwrap()+=1;adj.entry(e.source.as_str()).or_default().push(e.target.as_str());} let mut q=VecDeque::new(); for (id,d) in &indegree{if *d==0{q.push_back(*id)}} let mut visited=0; while let Some(id)=q.pop_front(){visited+=1;if let Some(next)=adj.get(id){for t in next{let d=indegree.get_mut(t).unwrap();*d-=1;if *d==0{q.push_back(*t)}}}} if visited!=g.nodes.len(){return Err(AppError::BadRequest("Automation graph contains a cycle".into()))}Ok(())
}
#[cfg(test)]
mod tests {
 use super::*;
