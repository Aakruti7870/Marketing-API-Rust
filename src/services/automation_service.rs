...
pub async fn get_run(pool:&sqlx::PgPool,workspace_id:Uuid,run_id:Uuid)->Result<Value,AppError>{
    ...
    let steps=sqlx::query("SELECT id,node_key,node_type,status,input,output,error,attempt,started_at,completed_at FROM automation_run_steps WHERE run_id=$1 ORDER BY started_at").bind(run_id).fetch_all(pool).await?;
