...
async fn http_node(http:&Client,c:&Value,input:&Value)->Result<NodeResult,AppError>{
 ...
 if resp.content_length().unwrap_or(0)>MAX_RESPONSE_BYTES as u64{return Err(AppError::ExternalService("Response exceeds 1 MB".into()))}let bytes=resp.bytes().await.map_err(|e|AppError::ExternalService(e.to_string()))?;if bytes.len()>MAX_RESPONSE_BYTES{return Err(AppError::ExternalService("Response exceeds 1 MB".into()))}
 let status=resp.status().as_u16();let v=serde_json::from_slice::<Value>(&bytes).unwrap_or_else(|_|json!({"body":String::from_utf8_lossy(&bytes)}));Ok(NodeResult{data:json!({"status":status,"response":v}),branch:None})
}
