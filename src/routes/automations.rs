...
async fn list(State(s):State<AppState>,t:TenantContext)->Result<Json<Value>,AppError>{Ok(json_success(automation_service::list_automations(&s.pool,t.workspace_id).await?,"Automations retrieved"))}
