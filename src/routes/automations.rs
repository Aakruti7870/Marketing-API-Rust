...
async fn remove(State(s):State<AppState>,t:TenantContext,Path(id):Path<Uuid>)->Result<impl IntoResponse,AppError>{
    ...
    let exists=sqlx::query("SELECT 1 FROM automations WHERE id=$1 AND workspace_id=$2")
        .bind(id).bind(t.workspace_id).fetch_optional(&mut *tx).await?;
    ...
    sqlx::query("DELETE FROM automation_logs WHERE run_id IN (SELECT id FROM automation_runs WHERE automation_id=$1)")
        .bind(id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM automation_run_steps WHERE run_id IN (SELECT id FROM automation_runs WHERE automation_id=$1)")
        .bind(id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM automation_runs WHERE automation_id=$1")
        .bind(id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM automation_schedules WHERE automation_id=$1")
        .bind(id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM automation_variables WHERE automation_id=$1")
        .bind(id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM automation_edges WHERE automation_id=$1")
        .bind(id).execute(&mut *tx).await?;
    ...
    sqlx::query("DELETE FROM automation_triggers WHERE automation_id=$1")
        .bind(id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM automations WHERE id=$1 AND workspace_id=$2")
        .bind(id).bind(t.workspace_id).execute(&mut *tx).await?;
    tx.commit().await?;
