...
use crate::services::workspace_service::{self, AddMemberDto, CreateWorkspaceDto, UpdateWorkspaceDto};
use crate::state::AppState;
use crate::utils::response::json_success;
use axum::{extract::{Path, State}, response::IntoResponse, routing::{delete, get}, Json, Router};
use uuid::Uuid;
...
async fn list_workspaces(State(state): State<AppState>, AuthenticatedUser(claims): AuthenticatedUser) -> Result<impl IntoResponse, AppError> {
...
async fn create_workspace(State(state): State<AppState>, AuthenticatedUser(claims): AuthenticatedUser, Json(dto): Json<CreateWorkspaceDto>) -> Result<impl IntoResponse, AppError> {
...
async fn get_current_workspace(State(state): State<AppState>, tenant: TenantContext) -> Result<impl IntoResponse, AppError> {
...
async fn update_current_workspace(State(state): State<AppState>, tenant: TenantContext, Json(dto): Json<UpdateWorkspaceDto>) -> Result<impl IntoResponse, AppError> {
...
async fn delete_current_workspace(State(state): State<AppState>, tenant: TenantContext) -> Result<impl IntoResponse, AppError> {
...
async fn list_workspace_members(State(state): State<AppState>, tenant: TenantContext) -> Result<impl IntoResponse, AppError> {
    let members = workspace_service::list_members(&state.pool, tenant.workspace_id).await?;
    Ok(json_success(members, "Members retrieved"))
}

async fn add_workspace_member(State(state): State<AppState>, tenant: TenantContext, Json(dto): Json<AddMemberDto>) -> Result<impl IntoResponse, AppError> {
    require_workspace_roles(&tenant.workspace_role, &["OWNER", "ADMIN"])?;
    let member = workspace_service::add_member(&state.pool, tenant.workspace_id, dto).await?;
    Ok(json_success(member, "Member invited"))
}

async fn remove_workspace_member(State(state): State<AppState>, tenant: TenantContext, Path(member_id): Path<Uuid>) -> Result<impl IntoResponse, AppError> {
    require_workspace_roles(&tenant.workspace_role, &["OWNER", "ADMIN"])?;
